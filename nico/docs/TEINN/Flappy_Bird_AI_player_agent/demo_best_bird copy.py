...
class Bird:
    ...
    def move(self):
        ...
        if self.vel > self.max_fall:
            self.vel = self.max_fall
        if self.vel < self.max_rise:
            self.vel = self.max_rise
...
def reset_round(mode):
    ...
    if mode == MODE_HUMAN_VS_AI:
        human_bird = Bird(BIRD_START_X, BIRD_START_Y, color="red", is_human=True)
    else:
        human_bird = None
...
def draw_ui(win, mode, ai_bird, human_bird):
    ...
    win.blit(font_big.render(title, True, (255, 255, 255)), (10, 10))

    if ai_bird:
        win.blit(font_small.render(f"AI Score: {ai_bird.score}", True, (120, 200, 255)), (10, 34))
        ai_state = "alive" if ai_bird.alive else "dead"
        win.blit(font_small.render(f"AI: {ai_state}", True, (120, 200, 255)), (10, 52))

    if human_bird:
        win.blit(font_small.render(f"Human Score: {human_bird.score}", True, (255, 120, 120)), (10, 70))
        human_state = "alive" if human_bird.alive else "dead"
        win.blit(font_small.render(f"Human: {human_state}", True, (255, 120, 120)), (10, 88))

    controls_1 = "1=AI only  2=Human vs AI  R=Restart"
    controls_2 = "SPACE=Flap human  ESC=Quit"
    win.blit(font_small.render(controls_1, True, (255, 255, 255)), (10, WIN_HEIGHT - 42))
    win.blit(font_small.render(controls_2, True, (255, 255, 255)), (10, WIN_HEIGHT - 24))
