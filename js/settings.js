;(() => {
  ...
  const withSolutionOnly = document.getElementById('with-solution-only')
  const excludeSuccess = document.getElementById('exclude-success')
  ...
  function getSubjectName (subject) {
    return subject.name ?? subject.code ?? String(subject.id ?? '')
  }

  function getSelectedSubjectsBySection () {
    const result = {}
    for (const [section, subjects] of Object.entries(state.subjects)) {
      const selected = []
  ...
  function getTagName (tag) {
    return tag.name ?? tag.label ?? String(tag.id ?? '')
  }
  ...
  function getTagColor (color) {
    ...
    const index = Number.isFinite(Number(color)) ? Number(color) : 0
  ...
  function renderSubjects () {
    ...
    for (const [section, subjects] of groups) {
      ...
      const hasSelectedSubject = subjects.some(subject =>
        state.selectedSubjects.has(`${section}/${subject}`)
      )
      details.open = hasSelectedSubject
      ...
      summary.appendChild(sectionCheckbox)
      summary.appendChild(sectionText)

      /*
       * Subjects.
       */
      const children = document.createElement('div')
      children.className = 'space-y-1 border-t border-slate-200 px-3 py-2'
      ...
      for (const subject of subjects) {
        ...
        checkbox.className =
          'h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500'
        checkbox.checked = state.selectedSubjects.has(key)
        ...
        checkbox.addEventListener('change', () => {
          ...
          updateSectionCheckbox(sectionCheckbox, subjectCheckboxes)
        })
      }
      ...
      sectionCheckbox.addEventListener('change', () => {
        for (const checkbox of subjectCheckboxes) {
          ...
          const subject = String(checkbox.value)
          const key = `${section}/${subject}`
      ...
      subjectsList.appendChild(details)
    }
  }

  function updateSectionCheckbox (sectionCheckbox, subjectCheckboxes) {
    const checked = subjectCheckboxes.filter(
      checkbox => checkbox.checked
    ).length
    sectionCheckbox.checked = checked === subjectCheckboxes.length
    sectionCheckbox.indeterminate =
      checked > 0 && checked < subjectCheckboxes.length
  }
  ...
  function renderTags () {
    ...
    const filtered = state.tags.filter(tag => {
      return getTagName(tag).toLocaleLowerCase().includes(search)
    })

    if (filtered.length === 0) {
      const message = document.createElement('p')
      message.className = 'text-sm text-slate-500'
      message.textContent = 'No matching tags.'
    ...
    for (const tag of filtered) {
      ...
      checkbox.type = 'checkbox'
      checkbox.value = id
      checkbox.checked = state.selectedTags.has(id)
      checkbox.className =
      ...
      sticker.className = 'rounded-full px-3 py-1 text-sm font-medium'
      sticker.style.backgroundColor = color
      sticker.style.color = textColor
      sticker.textContent = getTagName(tag)
  ...
  function applyRestoredSettings (settings) {
    const subjects = settings.subjects ?? settings.selectedSubjects ?? {}
    state.selectedSubjects = new Set()
    if (subjects && typeof subjects === 'object' && !Array.isArray(subjects)) {
      for (const [section, subjectList] of Object.entries(subjects)) {
        for (const subject of subjectList) {
          state.selectedSubjects.add(`${section}/${subject}`)
        }
      }
    }

    const selectedTags =
      settings.requiredTags ?? settings.selectedTags ?? settings.tags ?? []
    state.selectedTags = new Set(
      selectedTags.map(tag => {
        if (typeof tag === 'object') {
          return String(getTagId(tag))
        }
        return String(tag)
      })
    )

    state.withSolutionOnly = Boolean(
      settings.withSolutionOnly ?? settings.with_solution_only ?? false
    )
    state.excludeSuccess = Boolean(
      settings.excludeSuccess ?? settings.exclude_success ?? false
    )
    withSolutionOnly.checked = state.withSolutionOnly
    excludeSuccess.checked = state.excludeSuccess
  }

  function readSettingsFromDOM () {
    state.withSolutionOnly = withSolutionOnly.checked
    state.excludeSuccess = excludeSuccess.checked
  ...
  function getExamFolder (exam) {
    const season = getExamSeason(exam)
    const subtype = getExamSubtype(exam)
  ...
  function getExamBaseURL (exam) {
    return [
      ...
      encodeURIComponent(getExamFolder(exam))
    ].join('/')
  }

  function renderExerciseTags (tags) {
    exerciseTags.innerHTML = ''

    if (!tags || tags.length === 0) {
      exerciseTags.classList.add('hidden')
      return
    }

    exerciseTags.classList.remove('hidden')

    for (const tag of tags) {
      const sticker = document.createElement('span')

      const background = getTagColor(tag.color)
      const textColor = getTagTextColor(background)

      sticker.className = 'rounded-full px-3 py-1 text-sm font-medium shadow-sm'

      sticker.style.backgroundColor = background
      sticker.style.color = textColor
      sticker.textContent = getTagName(tag)

      exerciseTags.appendChild(sticker)
    }
  }
  ...
  function renderExam (exam) {
    ...
    const base = getExamBaseURL(exam)
    ...
    const links = [
      ...
      {
        label: exam.subject,
        href: `/exam-db/${encodeURIComponent(
          exam.section
        )}/${encodeURIComponent(exam.subject)}/`
      },
      {
    ...
    links.forEach((link, index) => {
      if (index > 0) {
        ...
        slash.textContent = '/'
        examBreadcrumb.appendChild(slash)
      }

      const anchor = document.createElement('a')
      anchor.className = 'underline hover:no-underline'
      anchor.href = link.href
      anchor.textContent = link.label

      examBreadcrumb.appendChild(anchor)
    })
  }
  ...
  function getAttachmentURL (attachment) {
    ...
    return attachment.location.startsWith('/')
      ? attachment.location
      : `/data/${attachment.location}`
  }

  function getAttachmentBackground (attachment) {
    const qualifier = String(attachment?.qualifier ?? '').toUpperCase()

    if (qualifier === 'CORRIGE' || qualifier === 'SOLUTION') {
      return {
  ...
  function getAttachmentAlt (attachment, exercise) {
    const number = exercise.exerciseIndex ?? exercise.exerciseNumber ?? ''

    const qualifier = String(attachment?.qualifier ?? '').toLowerCase()
  ...
  function createExerciseElement (exercise, exerciseIndex) {
    ...
    const section = fragment.querySelector('.exercise')

    section.dataset.exercise = String(
      exercise.exerciseIndex ?? exerciseIndex + 1
    )

    const number = fragment.querySelector('.exercise-number')

    number.textContent = String(exercise.exerciseIndex ?? exerciseIndex + 1)
    ...
    exercise.attachments.forEach(attachment => {
      ...
      const colors = getAttachmentBackground(attachment)

      imageButton.classList.add(colors.border, colors.background)

      const image = document.createElement('img')
      image.src = getAttachmentURL(attachment)
      image.alt = getAttachmentAlt(attachment, exercise)
      image.className =
     ...
     */
    fragment.querySelectorAll('.exercise-action').forEach(button => {
      button.addEventListener('click', () => {
        submitResult(button.dataset.action, exercise)
      })
    })
  ...
  function showExercise (exercise, replace = true) {
    if (!exercise) {
    ...
    const existingExercises = exercisesContainer.querySelectorAll('.exercise')

    const index = existingExercises.length

    const { element } = createExerciseElement(exercise, index)

    exercisesContainer.appendChild(element)

    const inserted = exercisesContainer.lastElementChild
  ...
  async function loadSubjects () {
    document.getElementById('clear-subjects').addEventListener('click', () => {
    ...
    try {
      ...
      message.textContent = 'Could not load subjects.'
      subjectsList.appendChild(message)
    }
  }
  ...
  async function loadTags () {
    ...
    try {
      ...
      const message = document.createElement('p')
      message.className = 'text-sm text-red-600'
      message.textContent = 'Could not load tags.'
      tagsList.appendChild(message)
    }
  }
  ...
  async function restoreSession () {
    try {
      const data = await getJSON(API.restore)

      applyRestoredSettings(data)
  ...
  async function startLearning () {
    ...
    try {
      ...
      console.error('Could not start learning:', error)
      showError('Could not start the learning session.')
    } finally {
      setBusy(false)
    }
  }

  async function submitResult (result, exercise) {
    if (state.busy) {
      return
    }
    ...
    try {
      const body = getNextRequestBody(result)
  ...
  withSolutionOnly.addEventListener('change', () => {
    state.withSolutionOnly = withSolutionOnly.checked
  })

  excludeSuccess.addEventListener('change', () => {
    state.excludeSuccess = excludeSuccess.checked
  })
   ...
   */
  document
    .querySelectorAll('#viewer-skip, #viewer-failed, #viewer-success')
    .forEach(button => {
      button.addEventListener('click', () => {
        submitResult(button.dataset.action, state.currentExercise)
      })
    })
  ...
  async function initialize () {
     ...
     */
    await Promise.all([loadSubjects(), loadTags(), restoreSession()])
  }
