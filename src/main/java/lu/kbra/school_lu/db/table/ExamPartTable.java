...
public abstract class ExamPartTable extends DeferredDatabaseTable<ExamPartData> {
	...
	@Query(distinct = true, tables = { @Table(typeName = ExamPartExamTable.class), @Table(typeName = ExamTable.class) })
	public abstract ExamPartData byExamAndPartName(@Param ExamData examData, @Param("{M:ExamPartTable:name}") @Nullable String name);
